angular.module('TaskMDL').controller('agregarClienteController', ['$scope', '$rootScope', '$mdDialog', '$location', 'clientesServices',
    function ($scope, $rootScope, $mdDialog, $location, clientesServices) {
        $scope.loading = false;
        $scope.existe = false;
        $scope.valid = true;
        $scope.usuario = $rootScope.Globals.CurrentUserCot;
        $scope.CompletarDatos = false;
        $scope.colonias = [];
        $scope.cliente = {
            vchNombre: '',
            vchEmail: '',
                    ...
                    clientesServices.ObtenerAgenteCobranza(request, function (responseCob) {
                        //clientesServices.ObtenerTipoCliente(request, function (responseTipoCliente) {
                            clientesServices.ObtenerTipoClienteFiscal(request, function (responseTipoClienteFiscal) {
                                ...
                                //$scope.precios = [{ intTipoPrecio: 0, vchNombre: 'No' }];
        ...
        $scope.AgregarCliente = function (cliente, ev) {
            $scope.loading = true;
            var Request = {
                Cliente: cliente,
                token: $rootScope.Globals.CurrentUserCot.Token,
                intUserID: $rootScope.Globals.CurrentUserCot.intUsuarioID,
                DatosCompletos: $scope.CompletarDatos
            };
            clientesServices.AgregarCliente(Request, function (response) {
                $scope.loading = false;
                if (response.Success)
                    $location.path('/clientes');//$mdDialog.hide(response);
                else {
